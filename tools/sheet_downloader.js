...
class SheetDownloader {
    ...
    /**
     * 명시한 스프레스시트의 시트 내용을 읽어 JSON 객체로 변환해줍니다.
     *@param spreadsheetId 스프레스 시트 ID
     *@param sheetName 시트 이름
     *@param filePath 저장할 JSON 파일 생략시 파일로 저장하지 않음)
     */
    async downloadToJson(spreadsheetId, sheetName, filePath = null) {
        ...
        //행 데이터(배열)를 객체로 변환합니다.4
        const object = this._rowsToObject(rows);

        //filePath를 명시했다면 지정한 파일로 저장5
        if (filePath) {
