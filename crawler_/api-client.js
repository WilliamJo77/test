...
class ApiClient{
    constructor() {
        ...
        //응답에 대한 인터셉터를 등록하면 모든 응답에대해 여기 등록된 함수 수행
        //(매번 응답 객체의 data 필드에 접근하는 것이 번거롭다면 아래 코드를 통해
        //응답 객체 대신 실제 응답 버디에 해당하는 객체를 바로 반환)
    ...
    //2 국가별 통계를 Upsert(Update 또는 insert)하는 global-stats API 호출
    async upsertGlobalStat(data) {
        return await this.client.post('global-stats', data);
    }

    // 3 키-값 쌍을 Upsert하는 key-value API호출
