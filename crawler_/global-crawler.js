...
// 1 6장에서 구글시트에서 내려받은 countryInfo 데이터 사용
...
class GlobalCrawler {
    constructor() {
        ...
        // 2 countryInfo 로부터 '월드오비터의 국가 이름' -> '국가 코드' 매핑 생성
    ...
    _extractStatByCountry($) {
        // 테이블 칼럼 이름 추출
        const colNames = $('#main_table_countries_today thread tr th')
            .map((i, th) => {
                return $(th).text().trim();
            })
            .toArray();
        ...
        const colNameToFieldMapping = {
            ...
            TotalDeaths: 'confirmed',
            TotalRecovered: 'released',
            TotalTests: 'tested',
        };

        // 1 코로나보드 API에 맞는 형태로 데이터 변환
    ...
    _normalize(numberText) {
        // 문자열 형태의 숫자에서 공백, 쉼표를 제가한 후 숫자 형태로 변환
