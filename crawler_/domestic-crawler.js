...
class DomesticCrawler {
    ...
    _extractBasicStats($) {
        ...
        titles.each((i, el) => {
            ...
            // 1 여기서 태그를 제외하고 순수 텍스트만 분리
            const titleTextEl = $(el)
            .contents() //요소 서브의 텍스트 노드를 포함한 모든 노드 변환
            .toArray()
        ...
        $('.data_table table').each((i, el) => {
            $(el)
                ...
                .each((j, row) => {
                    ...
                    _.forEach(mapping, (fileName, firstColumnText) => {
                        //현재 행의 첫번째 컬럼값이 mapping에 정의된 이름과
                        //동일한 경우에만 데이터 추출
